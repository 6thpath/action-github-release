...
export const releaseBody = ({ input_body_path, input_body }: Config): string | undefined => {
  return (input_body_path && readFileSync(input_body_path).toString('utf8')) || input_body
}
...
export const paths = (patterns: string[]): string[] => {
  return patterns.reduce<string[]>((acc, pattern) => {
    return acc.concat(sync(pattern).filter((path) => statSync(path).isFile()))
  }, [])
}
...
export const unmatchedPatterns = (patterns: string[]): string[] => {
  return patterns.reduce<string[]>((acc, pattern) => {
    return acc.concat(
      sync(pattern).filter((path) => statSync(path).isFile()).length == 0 ? [pattern] : []
    )
  }, [])
}
